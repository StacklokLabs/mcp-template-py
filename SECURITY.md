...
For each vulnerability, the security team will coordinate to create the fix and
release, and notify the rest of the community.
...
- These selected developers become the "Fix Team" (the fix team is often drawn
  from the project's MAINTAINERS).
...
- The security team will ensure all the container images are built, signed,
  publicly available, and functional.
- The security team will announce the new releases, the CVE number, severity,
  and impact, and the location of the images to get wide distribution and user
...
- The security team will send a retrospective of the process including details
  on everyone involved, the timeline of the process, links to relevant PRs that
